...
const updateCreditApi = async (details) => {
  ...
  const response = await axios.post(`${baseUrl}/updateClientCredits`, {
    ...details,
    cookie: cookieValue,
  });
  if (response.status == 201) {
...
const activeStatusApi = async (details) => {
  ...
  const response = await axios.post(`${baseUrl}/updateClientActivity`, {
    ...details,
    cookie: cookieValue,
  });
  if (response.status == 201) {
...
const updatePasswordApi = async (details) => {
  ...
  const response = await axios.post(`${baseUrl}/updateClientPassword`, {
    ...details,
    cookie: cookieValue,
  });
  if (response.status == 201) {
...
const deleteClientApi = async (details) => {
  ...
  const response = await axios.post(`${baseUrl}/deleteClient`, {
    ...details,
    cookie: cookieValue,
  });
...
const getTransactionsOnBasisOfPeriodApi = async (details) => {
  const cookieValue = Cookies.get("userToken");
  const response = await axios.post(
    `${baseUrl}/getTransanctionOnBasisOfDatePeriod`,
    { ...details, cookie: cookieValue }
  );
  return response;
};
...
export {
  baseUrl,
  getClientListApi,
  getTransactionsOnBasisOfPeriodApi,
  activeStatusApi,
